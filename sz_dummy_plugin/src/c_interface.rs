...
static DUMMY_ENCRYPTION: OnceLock<Mutex<Option<DummyEncryption>>> = OnceLock::new();

fn get_encryption() -> &'static Mutex<Option<DummyEncryption>> {
    DUMMY_ENCRYPTION.get_or_init(|| Mutex::new(None))
}

fn with_encryption<F, R>(f: F) -> Result<R, EncryptionError>
where
    F: FnOnce(&DummyEncryption) -> Result<R, EncryptionError>,
{
...
/// Initialize the Dummy encryption plugin
///
/// # Safety
